...
MessageHelper.prototype.fetchMessage = function (callback) {
    var _this = this;
    
    this.client.invokeApi("polling", {
        method: "post"
    }).done(function (results) {
        if (callback.success != null)
    		callback.success(results);
    	_this.fetchMessage(callback);
    }, function(err) {
    	console.log(err.message, err.toString(), err.request);
